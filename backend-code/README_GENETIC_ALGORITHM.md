...
# Create generator (input_file parameter is kept for compatibility but not used)
generator = GeneticTimetableGenerator(
    input_file="f2025.xlsx",  # Parameter kept for compatibility
    population_size=100,
    generations=300,
    mutation_rate=0.02,
    tournament_size=5,
...
**Note**: The genetic algorithm reads from the `validated/` folder containing CSV files generated by processing the original Excel input file. Ensure your input file has been processed through the validation step first.
...
- Reads 5 validated CSV files (Courses.csv, Faculty.csv, Rooms.csv, Time Slots.csv, Students.csv)
